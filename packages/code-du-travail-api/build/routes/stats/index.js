...
const MATOMO_URL = process.env.PIWIK_URL ?? "https://matomo.fabrique.social.gouv.fr";
router.get("/stats", async (ctx)=>{
    if (!MATOMO_SITE_ID && !MATOMO_URL) {
        ctx.throw(500, `There is no matomo`);
        return;
    }
    const { body: { aggregations  }  } = await elasticsearchClient.search({
        body: docsCountBody,
        index
    });
    let nbDocuments = 0;
    ...
    const promises = URLS.map(async (url)=>fetch(url).then(async (data)=>data.json()).catch((e)=>{
            console.error(e);
            return null;
        }));
