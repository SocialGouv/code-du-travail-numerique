...
// Remove Duplicates
const predicateFn = ({ _source: { source , slug  }  })=>`${getRouteBySource(source) || source}/${slug}`;
const removeDuplicate = (arr, predicate = predicateFn)=>arr.flatMap((a, index)=>{
...
const mergeDuplicate = (arr, predicate = predicateFn)=>{
    return arr.flatMap((a, index)=>{
        const prevItem = arr.find((item, i)=>predicate(item) === predicate(a) && i < index);
        if (prevItem) {
