...
const THRESHOLD = 90;
const NON_FUZZY_TOKENS = new Set([
    "cdd",
    "cdi",
    "csp"
]);
...
async function _getPrequalified() {
    ...
    const knownQueriesSet = response.body.hits.hits.reduce((queries, { _source: query  })=>{
...
const testFuzzyAllowed = (query, match)=>{
    ...
    const matchingNonFuzz = match.split(/\s+/).filter((token)=>NON_FUZZY_TOKENS.has(token));
    // if the matching variant contains non-fuzzy tokens,
    // ensure the query also contains it
    const queryTokens = query.split(/\s+/);
    if (matchingNonFuzz.length > 0) {
        return matchingNonFuzz.filter((token)=>queryTokens.includes(token)).length > 0;
    } else {
        return true;
    }
};
...
const getPrequalifiedResults = async (query)=>{
    ...
    // allow
    const knownQuery = query.length >= 3 && testMatch({
        allVariants,
        knownQueriesSet,
        query
    });
...
//# sourceMappingURL=searchPrequalifiedResults.js.map