 ...
 */ router.get("/search", async (ctx)=>{
    ...
    const skipPrequalifiedResults = ctx.query.skipSavedResults === "" || ctx.query.skipSavedResults === "true";
    // check prequalified requests
    const prequalifiedResults = !skipPrequalifiedResults && await getPrequalifiedResults(query);
    let documents = [];
    let articles = [];
    let themes = [];
    if (prequalifiedResults) {
        prequalifiedResults.forEach((item)=>item._source.algo = "pre-qualified");
        documents = prequalifiedResults.filter(({ _source: { source  }  })=>![
                SOURCES.CDT,
                SOURCES.THEMES
            ].includes(source));
        articles = prequalifiedResults.filter(({ _source: { source  }  })=>source === SOURCES.CDT);
        themes = prequalifiedResults.filter(({ _source: { source  }  })=>source === SOURCES.THEMES);
    }
    const searches = {};
    ...
    const query_vector = await vectorizeQuery(query.toLowerCase()).catch((error)=>{
        logger.error(error.message);
    });
    // if not enough prequalified results, we also trigger ES search
    if (!prequalifiedResults || prequalifiedResults.length < DEFAULT_RESULTS_NUMBER) {
    ...
    const semanticHitsFiltered = semanticHits.filter((item)=>item._score > SEMANTIC_THRESHOLD);
    // we merge prequalified + full text + semantic results
    const mergedSearchResults = mergePipe(fulltextHits, semanticHitsFiltered, size);
    documents.push(...mergedSearchResults);
    documents = removeDuplicate(documents);
    if (shouldRequestThemes) {
        const fulltextHits1 = extractHits(results[THEMES_ES]);
        const semanticHits1 = extractHits(results[THEMES_SEM]);
        fulltextHits1.forEach((item)=>item._source.algo = "fulltext");
        semanticHits1.forEach((item)=>item._source.algo = "semantic");
        themes = removeDuplicate(themes.concat(merge(fulltextHits1, semanticHits1, THEMES_RESULTS_NUMBER * 2)).slice(0, THEMES_RESULTS_NUMBER));
    }
    if (shouldRequestCdt) {
...
async function msearch({ client , searches  }) {
    ...
    const { body , error  } = await client.msearch({
        body: requests
    });
    if (error) {
        throw error;
    ...
    const results = keys.reduce((state, key, index)=>{
        ...
        if (resp.error) {
            logger.error(`Elastic search error : index ${index}, search key ${key} : ${JSON.stringify(resp.error, null, 2)}`);
        }
        state[key] = resp;
        return state;
    }, {});
