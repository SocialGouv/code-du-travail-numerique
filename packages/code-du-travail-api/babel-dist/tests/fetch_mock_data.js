...
var _cdtnSources = require("@socialgouv/cdtn-sources");
var _fs = require("fs");
var _path = require("path");
var _util = require("util");
var _v = require("../routes/v1.prefix");
var _versions = _interopRequireDefault(require("./versions.json"));

function _interopRequireDefault(obj) {
  return obj && obj.__esModule ? obj : { default: obj };
}
const writeFile = (0, _util.promisify)(_fs.writeFile);
...
const getDataFromUrl = (url) => {
  ...
  let source = (0, _cdtnSources.getSourceByRoute)(sourceRoute); // since theme routing has changed, we will keep this monkey patch for a while
  let trimmedSlug = slug;
  if (source === _cdtnSources.SOURCES.THEMES) {
  ...
  if (source === _cdtnSources.SOURCES.SHEET_MT && !slug.includes("#")) {
    source = _cdtnSources.SOURCES.SHEET_MT_PAGE;
  }
  return { slug: trimmedSlug, source };
};
const getDocumentByUrlQuery = (
  ...
  return {
    _source,
    query: { bool: { filter: [{ term: { slug } }, { term: { source } }] } },
    size: 1,
  };
};
/*
...
async function updateDocumentsData(slugs) {
  const index = `${ES_INDEX_PREFIX}-${_v.CDTN_ADMIN_VERSION}_${_cdtnElasticsearch.DOCUMENTS}`;
  ...
  try {
    ...
    data.push(
      {
        data: [
          {
            abbreviations: [],
            definition: "<p>Mesure d'urgence prise par pr\xE9caution.</p>",
            references: [
    ...
    await writeFile(
      (0, _path.join)(__dirname, "./cdtn_document.data.json"),
