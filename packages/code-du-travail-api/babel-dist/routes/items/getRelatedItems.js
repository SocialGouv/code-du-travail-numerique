...
const index = `${ES_INDEX_PREFIX}-${CDTN_ADMIN_VERSION}_${DOCUMENTS}`; // standard related items :
const sources = [
  SOURCES.TOOLS,
  SOURCES.SHEET_SP,
  SOURCES.SHEET_MT,
...
async function getCovisitedItems({ covisits }) {
  ...
  const esCovisits = await _elasticsearch.default
    .msearch({ body })
    .then((resp) =>
...
async function getSearchBasedItems({ title, settings }) {
  ...
  if (query_vector) {
    const semBody = getSemBody({
      query_vector, // we +1 the size to remove the document source that should match perfectly for the given vector
      size: MAX_RESULTS + 1,
      sources,
  ...
  } = await _elasticsearch.default.msearch({ body: requestBodies });
  const { hits: { hits: semanticHits } = { hits: [] } } = semResponse;
  const { hits: { hits: fullTextHits } = { hits: [] } } = esResponse;
  return utils
    .mergePipe(fullTextHits, semanticHits, MAX_RESULTS) // we filter fields and add some info about recommandation type for evaluation purpose
...
async function getRelatedItems({ title, settings, slug, covisits }) {
  ...
  const filteredItems = covisitedItems
    .concat(searchBasedItems) // avoid elements already visible within the item as fragments
