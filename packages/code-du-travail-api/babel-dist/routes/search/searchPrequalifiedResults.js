...
const THRESHOLD = 90;
const NON_FUZZY_TOKENS = new Set(["cdd", "cdi", "csp"]); // Preprocess query : remove accentuation and
...
async function _getPrequalified() {
  const { body: { count = 10000 } = {} } = await _elasticsearch.default.count({
    body: { query: prequalifiedQuery },
    index,
  });
  const response = await _elasticsearch.default.search({
    body: { query: prequalifiedQuery, size: count },
    index,
  });
  ...
  return { allVariants: Object.keys(knownQueriesSet), knownQueriesSet };
}
...
const testFuzzyAllowed = (query, match) => {
  ...
  const matchingNonFuzz = match
    .split(/\s+/)
    .filter((token) => NON_FUZZY_TOKENS.has(token)); // if the matching variant contains non-fuzzy tokens,
...
const testMatch = ({ query, knownQueriesSet, allVariants }) => {
  ...
  if (results && results.length > 1) {
    ...
    const bestScore = results[0][1]; // ensure match is valid
...
const getPrequalifiedResults = async (query) => {
  const { knownQueriesSet, allVariants } = await getPrequalified(); // allow
  const knownQuery =
  ...
  if (knownQuery && knownQuery.refs && knownQuery.refs.length > 0) {
    ...
    return knownQuery.refs.map((ref) => ({ _source: ref }));
  }
  return false;
};
