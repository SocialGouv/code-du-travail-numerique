 ...
 */ router.get("/search", async (ctx) => {
  ...
  const skipPrequalifiedResults =
    ctx.query.skipSavedResults === "" || ctx.query.skipSavedResults === "true"; // check prequalified requests
  ...
  if (shouldRequestThemes) {
    ...
    searches[THEMES_ES] = [
      ...
      getRelatedThemesBody({ query, size: themeNumber }),
    ];
    if (query_vector) {
      searches[THEMES_SEM] = [
        { index },
  ...
  if (shouldRequestCdt) {
    ...
    searches[CDT_ES] = [
      { index },
      getRelatedArticlesBody({ query, size: cdtNumber }),
    ];
  }
  const results = await msearch({ client: _elasticsearch.default, searches });
  ...
  semanticHits.forEach((item) => (item._source.algo = "semantic")); // we only consider semantic results above a given threshold
  ...
  ctx.body = {
    ...
    documents: documents.map(({ _score, _source }) => ({ _score, ..._source })), // we add source prop since some result might come from dedicataed themes index
...
async function msearch({ client, searches }) {
  const requests = [];
  const keys = []; // return an empty object if we receive an empty object
