...
const suppressedFiches = prevFiches.filter(
  prevFiche => !nextFiches.some(nextFiche => nextFiche.url === prevFiche.url)
);
// Toutes les fiches suivantes qui ne sont pas dans les fiches précédentes
const addedFiches = nextFiches.filter(
  nextFiche => !prevFiches.some(prevFiche => prevFiche.url === nextFiche.url)
);
