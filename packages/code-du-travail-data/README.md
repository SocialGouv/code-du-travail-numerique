...
### Indexation des document dans elasticsearch

Les données utilisées par le site sont issues de packages de données (@socialgouv/\*-data).
...
$ NLP_URL=https://preprod-serving-ml.dev2.fabrique.social.gouv.fr yarn workspace @cdt/data ingest
```

Le script va créer 2 index elasticsearch( un dédié aux documents, l'autre aux suggestions).
