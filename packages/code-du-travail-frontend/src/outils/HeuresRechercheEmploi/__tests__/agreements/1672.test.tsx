    ...
    it("should display expected answer", () => {
      expect(screen.queryAllByText(/le temps et dans les conditions décidées par l'employeur, qui lui accordent des facilités d'absence/g)[0]).toBeInTheDocument();
          expect(screen.queryAllByText(/Le salaire n'est pas maintenu./g)[0]).toBeInTheDocument();
          expect(screen.queryAllByText(/Les heures d'absence autorisée sont accordées au salarié qui n'a pas encore retrouvé un emploi. L'employeur en définit les modalités et la rémunération./g)[0]).toBeInTheDocument();
    ...
    it("should display expected answer", () => {
      expect(screen.queryAllByText(/50 heures par mois maximum/g)[0]).toBeInTheDocument();
          expect(screen.queryAllByText(/Le salaire est maintenu./g)[0]).toBeInTheDocument();
          expect(screen.queryAllByText(/Ces heures peuvent être groupées en tout ou en partie, avec l'accord de l'employeur. Cette autorisation d'absence prend fin lorsque le salarié a retrouvé un emploi./g)[0]).toBeInTheDocument();
