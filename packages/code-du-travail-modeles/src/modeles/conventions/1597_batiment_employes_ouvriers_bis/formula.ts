...
export type Formula1597Props = {
  hasMoreThan55Years: boolean;
  seniority: number;
  refSalary: number;
};

export class Formula1597
  implements IFormula<SupportedCcIndemniteLicenciement.IDCC1597> {
  computeFormula({
    seniority,
    refSalary,
    hasMoreThan55Years,
  }: FormulaProps<SupportedCcIndemniteLicenciement.IDCC1597>): Formula {
    let formula = "";
    const explanations = [];
