...
describe("Formula", () => {
  ...
  describe("Check formula for CC when fold back to droit du travail", () => {
    test.each`
      ...
      ${50} | ${5}      | ${"(1/4 * Sref * A1)"}                     | ${["A1 : Ancienneté totale (5 ans)", "Sref : Salaire de référence (2300 €)"]}
      ${50} | ${16.4}   | ${"(1/4 * Sref * A1) + (1/3 * Sref * A2)"} | ${["A1 : Ancienneté totale (16.4 ans)", "A2: Années d'ancienneté au-delà de 10 ans (6.4 ans)", "Sref : Salaire de référence (2300 €)"]}
    `(
      "Avec une séniorité de $seniority ans: $expectedFormula",
      ({ seniority }) => {
        const situation = engine.setSituation({
        ...
        expect(formule.formula).toEqual(
