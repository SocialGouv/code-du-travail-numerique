...
data_path = os.path.join(os.path.dirname(os.path.abspath(__name__)), "data")
...
with open(dump_path, "r") as dump:
    ...
    documents = [
        dict(d, text="") if d.get("source") == "themes" else d for d in documents
    ]
    for document in documents:
        if (
            document.get("source") != "code_du_travail"
            and document.get("source") != "glossary"
            and document.get("source") != "prequalified"
            and document.get("source") != "highlights"
        ):
